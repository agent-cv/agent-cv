...
import type { Inventory, OutputRenderer, Project, YearlyTheme } from "../types.ts";
...
export class MarkdownRenderer implements OutputRenderer {
  ...
  render(inventory: Inventory, selectedIds: string[]): string {
    ...
    lines.push(`*Generated by [agent-cv](https://agent-cv.dev) on ${new Date().toISOString().split("T")[0]}*`, "");
    lines.push("---", "");

    const yearlyThemeByYear = new Map<string, YearlyTheme>();
    if (insights?.yearlyThemes) {
      for (const yt of insights.yearlyThemes) {
        yearlyThemeByYear.set(yt.year, yt);
      }
    }
    ...
    for (const year of sortedYears) {
      ...
      const yt = yearlyThemeByYear.get(year);
      if (yt?.studiedProjects && yt.studiedProjects.length > 0) {
        lines.push("### Explored / studied (clones)");
        lines.push("");
        for (const name of yt.studiedProjects) {
          lines.push(`- ${name}`);
        }
        lines.push("");
      }
      if (yt?.openSourceContributions && yt.openSourceContributions.length > 0) {
        lines.push("### Open source (contributions upstream)");
        lines.push("");
        for (const o of yt.openSourceContributions) {
          const up = `https://github.com/${o.upstream}`;
          lines.push(
            `- **${o.displayName}** → [\`${o.upstream}\`](${up}) (${o.pullRequestCount} ${plural(o.pullRequestCount, "PR", "PRs")})`
          );
        }
        lines.push("");
      }
