...
/**
 * Remove the published portfolio for the authenticated user.
 */
export async function unpublishPortfolio(jwt: string): Promise<void> {
  const client = createAgentCvTrpcClient(jwt);
  try {
    await client.publish.unpublish.mutate();
  } catch (e) {
    if (e instanceof TRPCClientError) {
      if (isTrpcUnauthorized(e)) {
        throw new Error("AUTH_EXPIRED");
      }
      throw new Error(e.message || "Unpublish failed");
    }
    throw e;
  }
}
...
/**
 * Publish inventory to the API (tRPC publish.upsert).
 */
export async function publishToApi(
  ...
  const client = createAgentCvTrpcClient(jwt);
  try {
    return await client.publish.upsert.mutate(payload);
  } catch (e) {
    if (e instanceof TRPCClientError) {
      if (isTrpcUnauthorized(e)) {
        throw new Error("AUTH_EXPIRED");
      ...
      const shape = e.shape as { data?: { zodError?: unknown } } | undefined;
      if (shape?.data?.zodError) {
        msg += ` ${JSON.stringify(shape.data.zodError)}`;
      }
      throw new Error(msg);
    }
    throw e;
