...
import { statsFlowMachine } from "./stats.machine.ts";
...
export default function Stats({ args }: Props) {
  ...
  const [state] = useMachine(statsFlowMachine, { input: { directory } });
  ...
  if (state.matches("running")) {
    return (
      <Text color="yellow">
        {directory ? `Scanning ${directory}...` : "Loading inventory..."}
      </Text>
    );
  }
  ...
  return <StatsBody inventory={inventory} />;
}

function StatsBody({ inventory }: { inventory: Inventory }) {
  ...
  return (
    <Box flexDirection="column">
      ...
      <Text>
        {myProjects} your projects ({totalInInventory} total in inventory) | {sortedLangs.length}{" "}
        languages | {years.find((y) => y !== "Unknown") || "?"} —{" "}
        {[...years].reverse().find((y) => y !== "Unknown") || "?"}
      </Text>
    </Box>
  );
}
